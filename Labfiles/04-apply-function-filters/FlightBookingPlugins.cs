...
{
    private const string FilePath = "Labfiles/04-apply-function-filters/flights.json";
    ...
    [Description("Search for available flights based on the destination and departure date in the format YYYY-MM-DD.")]
    ...
    {
        var availableFlights = flights.FindAll(flights =>
            flights.Destination.Equals(destination, StringComparison.OrdinalIgnoreCase) &&
            flights.DepartureDate.Equals(departureDate, StringComparison.OrdinalIgnoreCase) &&
            !flights.IsBooked);
        return availableFlights.ToList();
    }
